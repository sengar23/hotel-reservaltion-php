...
$ip_confirmation=mysqli_query("select * from reservation where confirmation='$confirmation'");
...
echo 'Rooms: <br>';
$ip_room=mysqli_query("select * from rooinventory where confirmation='$confirmation'");
while($rowa = mysqli_fetch_array($ip_room))
	{
	$qty=$rowa['qty'];
	$room=$rowa['room'];
	$ip_roomdetails=mysqli_query("select * from internet_shop where id='$room'");
